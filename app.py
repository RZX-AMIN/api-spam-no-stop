import requests, os, sys, jwt, pickle, json, binascii, time, urllib3, xKEys, base64, datetime, re, socket, threading
import asyncio
...
def run_api():
    ...
    print("🌐 بدء تشغيل API...")
    try:
        # استخدام threaded=True لتحسين الأداء
        app.run(host='0.0.0.0', port=5000, debug=False, threaded=True)
    except Exception as e:
        print(f"❌ خطأ في تشغيل API: {e}")
        # إعادة التشغيل بعد 10 ثواني في حالة الفشل
        time.sleep(10)
        run_api()
...
class FF_CLient():
    ...
    def ToKen_GeneRaTe(self , Access_ToKen , Access_Uid):
        ...
        self.dT = self.dT.replace(b'2025-10-29 02:14:58' , str(datetime.now())[:-7].encode())        
        ...
        try:
            # إصلاح مشكلة EnC_AEs - استخدام try-catch للتعامل مع الأخطاء
...
def StarT_SerVer():
    ...
    for account in ACCOUNTS:
        ...
        thread.start()
        time.sleep(3)  # تأخير أطول بين بدء كل حساب
